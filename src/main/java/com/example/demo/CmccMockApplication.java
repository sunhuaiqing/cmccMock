...
public class CmccMockApplication {
    ...
    public static final String securtyKey = "b6112e80780357e3a35af0cc8d7996a33AB17DB95999C1BE204F2E";

    // 设置log
    public static final Logger logger = LoggerFactory.getLogger(CmccMockApplication.class);
    ...
    public BaseList getBaseList() {
        ...
        baseList.setMessage("业务处理成功Moke");
        return baseList;
    }
    ...
    public @ResponseBody String jfInter(HttpServletRequest request) throws Exception {
        ...
        System.out.println("@@@interCode：" + interCode);
...
//		System.out.println("@@@4 hm :"+new MD5Encryption().getMD5(s2.toString()));
    ...
    public String jf0006(HttpServletRequest request, BaseList baseList) throws Exception {
        ...
        if (smsCodeId.equals(optCode)) {
            responseMap.put("returnCode", "0000");
            responseMap.put("message", "操作成功mock扣减验证OK");
            System.out.println("jf0006请求入参smsCodeId" + smsCodeId + ",optCode:" + optCode + ",两者值相等，验证通过。");
        } else {
            responseMap.put("returnCode", "9999");
            responseMap.put("message", "移动B025错误短信验证码错误mock");
            System.out.println("jf0006请求入参smsCodeId" + smsCodeId + ",optCode:" + optCode + ",两者值不相等，验证不通过。");
        }
